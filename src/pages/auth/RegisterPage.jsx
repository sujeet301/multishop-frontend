...
function CountdownTimer({ seconds, onExpire }) {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      setLeft((prev) => {
        if (prev <= 1) { clearInterval(iv); onExpire?.(); return 0; }
        return prev - 1;
      });
    }, 1000);
...
function PasswordStrength({ password }) {
  ...
  const textColors = ["", "text-red-500", "text-amber-500", "text-yellow-500", "text-green-500"];
    ...
    <div className="mt-2">
      <div className="flex gap-1">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className={`flex-1 h-1 rounded-full transition-all duration-300 ${i <= score ? colors[score] : "bg-gray-200 dark:bg-gray-700"}`}
          />
        ))}
      </div>
...
export default function RegisterPage() {
  ...
  const handleVerifyOTP = async (currentOtp) => {
    ...
    setLoading(true);
    try {
      const res = await api.post("/auth/register/verify-otp", { email: sentEmail, otp: code });
  ...
  return (
    <div className="min-h-screen flex bg-gray-50 dark:bg-gray-950">
      ...
      <div className="hidden lg:flex flex-col justify-center items-center flex-1 relative overflow-hidden"
        ...
        {["🛍️","✨","🎁","💎","⚡","🌟"].map((emoji, i) => (
          <motion.span key={i} className="absolute text-4xl opacity-20 select-none pointer-events-none"
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7 }} className="relative z-10 text-center text-white px-8">
          ...
          <div className="grid grid-cols-2 gap-3 max-w-xs mx-auto">
            {["10M+ Products", "50K+ Sellers", "Free Returns", "Secure Payments"].map((f) => (
              <div key={f} className="bg-white/15 backdrop-blur-sm rounded-xl p-3 text-sm border border-white/25 font-semibold">
                {f}
              </div>
            ))}
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              <motion.div key="step1"
                ...
                <p className="text-gray-500 dark:text-gray-400 text-sm mb-6">Free forever · Takes less than a minute</p>
                ...
                <form onSubmit={handleSendOTP} className="space-y-4">
                  ...
                  <motion.button
                    ...
                    disabled={loading}
                    whileTap={{ scale: 0.98 }}
                    className="w-full py-3 rounded-xl text-white font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-60 transition-all shadow-md hover:shadow-lg"
              ...
              <motion.div key="step2"
                ...
                <div className="space-y-5">
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="w-full py-3 rounded-xl text-white font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-50 transition-all shadow-md hover:shadow-lg"
                    style={{ background: "linear-gradient(135deg,#6366f1,#4f46e5)" }}
                  >
